...
learned reconstruction branch supplies scheduled TPLS supervision: its loss shapes the shared lifted
representation during training. Released checkpoints retain this branch, and the current `forward`
returns `aux_recon`; the segmentation logits never consume that reconstructed output, and the branch
can be pruned for deployment. Official code for **"The Lift
...
The released WBC checkpoints use the explicit **`paper-legacy-v1`** protocol. It
...
to the public checkpoints. Changing an existing prepared WBC tree requires
`--force`; otherwise the preparer fails rather than mixing or guessing protocols.

## Pretrained weights
The ten released STSF+TPLS checkpoints (three datasets × three seeds, plus the optical-bench
checkpoint) are on the Hugging Face Hub as inference-only exports — optimizer and
...
its exact byte count, Git LFS SHA-256 object ID, and legacy MD5.
...
Licensing splits across the three artifacts: source code here is MIT ([LICENSE](LICENSE)); the
released weights are CC BY 4.0, declared on the Hugging Face model card, which is also the canonical
description of the checkpoints' operating envelope; datasets keep source-specific terms
...
followed by the per-dataset image-domain segmenter. The released weights cover the
main model only, so train this baseline first (Train section above) to produce
`checkpoints/rev_carvana_traditional_s42/best.pth`:
```bash
...
realization to both lifts and reports the linear relative perturbations, the first-order
orthogonal-row prediction, and the actual shift after each arm's min-max normalization, with
per-sample values retained in JSON:
...
single-seed compatibility mode. The first-order orthogonal-row approximation is reported
alongside the realized linear and post-normalization shifts.
...
pull. The released diagnostic protocol uses
...
- **Aggregating many runs.** `scripts/merge_results.py` collects clean and noise JSONs from
  ...
  runs at seed 42 (their directory names carry no `_s42` infix; see the comment in `merge_results.py`'s `noise_eval_row`).
