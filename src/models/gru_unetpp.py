...
class GRUUNetPP(BaseSegmentationModel):
    ...
    def __init__(self,
        ...
        # NOTE: INERT / legacy key, not a knob: no post-hoc residual gate is implemented and no code in this class reads the attribute;
        ...
        # `use_residual: false`, the released rev_*_tpls among them — they match training exactly, so released checkpoints are unaffected;
        ...
        # NOTE: INERT in this class (same status as use_residual above): both belonged to the original
        ...
        # original meanlast + Linear + FSRCNN. It was chosen on the strength of the lift ablation;
    ...
    # NOTE: SHARED TAIL — duplicated verbatim in src/models/lift_unetpp.py
    #    (_compute_seq_len · _build_adaptive_upsample · _init_weights · the ms_enhance/attn/aux/UNet++ tail).
    #    LiftUNetPP(lift='attn') must stay numerically identical to this model so the released
    #    lift ablation is a fair, capacity-matched comparison. EDIT BOTH FILES TOGETHER — a silent
