...
def test_stepwise_phase_counts_are_exact():
    assert SegmentationTrainer._stepwise_boundaries(9 * 40, 0.3, 0.3) == (108, 216)


def test_noise_amplification_matches_first_order_for_orthogonal_rows():
    ...
    assert record["linear_ratio"] == pytest.approx(record["first_order_ratio"], rel=1e-6)
